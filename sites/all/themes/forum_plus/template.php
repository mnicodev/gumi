...
function forum_plus_preprocess_forums(&$variables) {
	//print_r($variables);
	unset($variables["forum_legend"]);
	unset($variables["forum_statistics"]);
	//exit;
}
	

function forum_plus_breadcrumb($variables) {
   $breadcrumb = $variables['breadcrumb'];
	unset($breadcrumb[count($breadcrumb)-2]);
  if (!empty($breadcrumb)) {
    // Provide a navigational heading to give context for breadcrumb links to
    // screen-reader users. Make the heading invisible with .element-invisible.
    //$output = '<h2 class="element-invisible">' . t('You are here') . '</h2>';
	$output = '';
    $output .= '<div class="breadcrumb">' . implode(' <img src="/'.drupal_get_path('theme','forum_plus').'/images/point_breadcrumb.png" /> ', $breadcrumb) . '</div>';
    return $output;
  }
}
...
function forum_plus_theme() {
  ...
  $items['render_panel'] = array(
	...
	$items['user_login'] = array(
    	'template' => 'tpl/user_login',
	    'render element' => 'form',
  	);
	$items['user_register_form'] = array(
    	'template' => 'tpl/user_register',
		'render element' => 'form',
		 'preprocess functions' => array('forum_plus_preprocess_user_register')
	 );

	$items['user_pass'] = array(
	    'render element' => 'form',
    	'template' => 'tpl/user_password',
	    'preprocess functions' => array('forum_plus_preprocess_user_pass'),
  	);
  return $items;
}
...
function forum_plus_preprocess_user_pass(&$variables) {
  $variables['intro_text'] = t('request a new password');
  $variables['form']["name"]["#prefix"]="";
}


function forum_plus_preprocess_user_login(&$variables) {
	//print_r($variables);
  $variables['intro_text'] = t('sign in');
  $variables['form']["name"]["#prefix"]="";
}
function forum_plus_preprocess_user_register(&$variables) {
	$variables['intro_text'] = t('create a player account');
	//$variables['form']["account"]["name"]["#prefix"]; //t("register");
